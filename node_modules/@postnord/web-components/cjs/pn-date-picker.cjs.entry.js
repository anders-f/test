...
const PnDatePicker = class {
  ...
  render() {
    return (index.h(index.Host, null, index.h("div", { class: "pn-date-input-container" }, index.h("input", { type: "text", value: this.startDate ? this.formatDate(this.startDate) : null, onInput: e => this.startInputHandler(e), onFocusin: () => (this.open = true), placeholder: this.placeholder }), this.range && index.h("pn-icon", { class: this.arrowDirection(), symbol: "arrow-right", color: "blue700" }), this.range && (index.h("input", { type: "text", value: this.formatDate(this.endDate), onInput: e => this.endInputHandler(e), onFocusin: () => (this.open = true), placeholder: this.endPlaceholder })), index.h("button", { class: "pn-date-calendar", type: "button", onClick: () => (this.open = !this.open) }, index.h("pn-icon", { symbol: "calendar", color: "blue700" }))), index.h("div", { class: this.getCalendarClasses(), "data-open": this.open }, index.h("div", { class: "calendar" }, this.years.map(year => (index.h("div", { "data-year": year }, this.months.map((monthString, month) => (index.h("div", { "data-month": monthString }, index.h("h3", { class: "pn-date-picker-month" }, monthString, " - ", year), index.h("div", { class: "month" }, this.week.map(day => (index.h("h4", { class: "weekday pn-2xs" }, day))), this.daysInMonth(month, year).map(day => (index.h("button", { class: "day", style: this.calcWeekdayStart(year, month, day), "data-day": day, "data-today": this.checkToday(year, month, day), "data-selected": this.calculateSelected(year, month, day), onClick: event => this.send({ year, month, day }, event), onPointerOver: event => this.send({ year, month, day }, event), tabindex: "-1", disabled: this.shouldDisable(year, month, day), type: "button" }, index.h("span", { class: "number" }, day)))))))))))))));
  }
...
exports.pn_date_picker = PnDatePicker;
