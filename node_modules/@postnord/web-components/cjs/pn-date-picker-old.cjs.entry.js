...
const PnDatePickerOld = class {
  constructor(hostRef) {
    ...
    this.dateerror = index.createEvent(this, "dateerror", 7);
  }
  componentWillLoad() {
    this.isInit = true;
  ...
  render() {
    const context = this;
    const trapKeyStrokes = helpers.debounce(function (event) {
      context.value = event.target.value;
    }, 400, false);
    const noInput = function (event) {
      event.preventDefault();
    };
    return (index.h("div", { class: "datepicker-container" }, index.h("div", { class: "datepicker-input-container" }, index.h("input", { type: "text", class: "date-input", value: this.value, onFocus: this.openDatepicker.bind(this), onKeyDown: this.disableInput ? noInput : trapKeyStrokes.bind(this), disabled: this.disabled }), index.h("pn-icon", { symbol: "calendar", onClick: this.openDatepicker.bind(this), color: this.disabled ? 'gray400' : 'black' }), index.h("label", null, this.label)), index.h("div", { class: "datepicker-calendar-container" }, index.h("pn-calendar", { value: this.value, min: this.min, max: this.max, language: this.language, "disable-days": this.disableDays, "disable-weekends": this.disableWeekends, "start-of-week": this.startOfWeek, onMonthchanged: this.onMonthChanged.bind(this) }))));
  }
...
exports.pn_date_picker_old = PnDatePickerOld;
