...
const PnZipCodeSearch = class {
  constructor(hostRef) {
    ...
    this.searchsuccessful = index.createEvent(this, "searchsuccessful", 7);
  ...
  renderSearchResult() {
    if (!this.loading && !this.error && this.showResult) {
      return (index.h("div", { class: "search-results" }, index.h("div", { class: "text-row" }, index.h("h3", null, this.getTranslation('CLOSEST_DELIVERY_DATE', this.marketwebLanguage || this.language), " "), index.h("h2", { class: "delivery-date" }, this.zipCodeSearchResult.delivery)), index.h("div", { class: "text-row" }, index.h("h3", null, this.getTranslation('NEXT_DELIVERY_DATE', this.marketwebLanguage || this.language), " "), index.h("h2", { class: "delivery-date" }, this.zipCodeSearchResult.upcoming)), index.h("div", { class: "text-row" }, index.h("h3", null, this.getTranslation('ZIP_CODE', this.marketwebLanguage || this.language)), index.h("h2", null, " ", this.zipCodeSearchResult.postalCode)), index.h("h3", null, this.zipCodeSearchResult.city)));
    }
  }
  renderErrorMessage() {
    if (this.error && !this.loading) {
      return (index.h("p", { class: "error-text pn-tiny" }, this.getTranslation(this.errorMessage, this.marketwebLanguage || this.language)));
    }
  }
  render() {
    return (index.h(index.Host, null, index.h("pn-search-field", { loading: this.loading, button: "none", placeholder: this.getTranslation('PLACEHOLDER_TEXT', this.marketwebLanguage || this.language), onSearch: (e) => this.handleSearch(e) }), this.renderSearchResult(), this.renderErrorMessage()));
  }
};
PnZipCodeSearch.style = pnZipcodeSearchCss;

exports.pn_zipcode_search = PnZipCodeSearch;
