...
const PnOcrSearch = class {
  ...
  renderSearchResult() {
    if (!this.loading && !this.error && this.searchSuccess) {
      return (h("div", { class: "search-results" }, this.getTranslation('SEARCH_SUCCESS', this.language), ": ", h("b", null, this.ocrNumber)));
    }
  }
  renderErrorMessage() {
    if (this.invalidOcrNumber && !this.loading) {
      return (h("div", { class: "invalid-text-container" }, h("p", { class: "invalid-ocr" }, this.getTranslation('ERROR_INVALID_LICENSE_NUMBER', this.language))));
    }
    if (this.error && !this.loading) {
      return (h("div", { class: "error-text-container" }, h("p", { class: "bold" }, this.getTranslation('ERROR_NO_RESULTS', this.language)), h("p", { class: "bold" }, this.getTranslation('ERROR_REASON_HEADER', this.language)), h("p", null, this.getTranslation('ERROR_REASON_ONE', this.language)), h("p", null, this.getTranslation('ERROR_REASON_TWO', this.language))));
    }
  }
  render() {
    return (h(Host, null, h("pn-search-field", { loading: this.loading, label: this.getTranslation('SEARCH_BUTTON', this.language), button: "", placeholder: this.getTranslation('PLACEHOLDER_TEXT', this.language), onSearch: (e) => this.handleSearch(e) }), this.renderSearchResult(), this.renderErrorMessage()));
  }
};
PnOcrSearch.style = pnOcrSearchCss;

export { PnOcrSearch as pn_ocr_search };
